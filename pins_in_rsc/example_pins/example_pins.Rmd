...
So you think you're hot stuff and off and running?  For now, perhaps.  But let's say you want to publish something to RStudio Connect (like this RMD file), which creates or updates a pin. You might do this because you want to use Connect's scheduling feature to make an ETL (extract, transform, and load) process that keeps your pin up-to-date.  That way, downstream apps or analyses stay current while pointing to this updated pin, and you don't have to re-deploy that app or analysis every time the data updates!  Now _that's_ working smart!
