...
Think about some of your workflows.  Are you: # I think this should come first before the furniture analogy. Also not sure if furniture analogy is needed.
...
# What are Pins?
Just like you’d pin a note (or a picture of your BFF) to a physical cork board, `pins` lets you pin an R or Python object to virtual board where you and others access it.  The virtual board can be on RStudio Connect, S3, Google Cloud, or your own website, to name a few.
...
1. Have RStudio Connect v 1.7.8 or higher.^[If you're not yet at this version but are keen to get started with `pins`, talk to your R Admin and refer to https://docs.rstudio.com/connect/admin/server-management/#upgrading] # This footnote is split between the two columns and makes it a little hard to read
...
1. For convenience, save your RStudio Connect server address as a system environment variable in your .RProfile as well. Example: `Sys.setenv("CONNECT_SERVER" = "https://your-server-address.com/")`{.R} # It's important that there is an extra "return" character added (or an empty line) after the last line of the .RProfile file or else the last line won't be executed.
1. Save and close the file.
1. Restart R (shift + cmd + F10). # is this the same keyboard shortcut for windows?
...
```{r}
my_data <- iris #for example (there has been a lot of backlash regarding use of the iris dataset, maybe use penguins! https://github.com/allisonhorst/palmerpenguins...or mtcars for something built into R
#pin object
pins::pin(my_data, 
    name = "my_data",
