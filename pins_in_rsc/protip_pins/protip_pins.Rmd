...
1. Return to the RStudio IDE and save your API key as a system environment variable in your .RProfile file:
    ...
    a. \raggedright In the .RProfile, insert `Sys.setenv( "CONNECT_API_KEY" = "paste key value")`{.R}.
...
**Install `pins`.** The `pins` package is available on CRAN, and was at version `r packageVersion("pins")` at the time of this writing. Install `pins` with \raggedright `install.packages("pins")`{.R}.
